...
//	const parent_elems = JSON.parse(data.majaselections).map((obj) => ({ _id: obj._id, itemId: obj.itemId, text: obj.text }))
	...
	import AddMidselModal from '../mid/AddMinselModal.svelte'
	import EditMidselModal from '../mid/EditMinselModal.svelte'
	import AddNewMidselModal from '../mid/AddNewMidselModal.svelte'
	import InsertManyCsvModal from '../mid/InsertManyCsvModal.svelte'
	import DeleteManyModal from '../mid/DeleteManyModal.svelte'
	import DeleteMidselModal from '../mid/DeleteMidselModal.svelte'
...
<section>
	...
	<div class="container" style="width: 94vw;margin-left: -8px;">
		<div class="card" style="width: 100%; display: flex; flex-flow: column; flex-wrap: nowrap; background: radial-gradient(#76b2fe, #b69efe);">
			<div class="card-title-header" style="padding: 8px;">
				<span style="font-size: 1.25rem; color: #fff;">{row.text}</span>
			</div>
			<div class="card-contents" style="display: flex; flex-flow: column; padding-left: 1.25rem; padding-right: 2rem;">
				<span style="display: inline-block; font-size: 1.25rem; color: #fff;">itemId: {row.itemId}</span>
				<span style="display: inline-block; font-size: 1rem; color: #fff;">大分類: {row.parentId.text}</span>
			</div>
			<div class="card-footer" style="display: grid; grid-template-columns: repeat (12, 1fr); gap: 4px; align-items: center; justify-content: end;">
				<div style="grid-column: 10 / span 1; display: flex; justify-content: center; align-items: center;">
					<img src="/src/lib/assets/icons/pencil-alt.svg" alt="edit" style="height: 1.25rem;" />
				</div>
				<div style="grid-column: 11 / span 1; display: flex; justify-content: center; align-items: center;">
					<img src="/src/lib/assets/icons/trash-o.svg" alt="trash" style="height: 1.25rem;" />
				</div>
			</div>
		</div>
	</div>
